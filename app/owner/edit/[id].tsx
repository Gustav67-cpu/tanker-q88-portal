...
function fromVessel(v: Vessel): FormState {
  const s = (n: number | string | null) => (n == null ? "" : String(n));
  return {
    vessel_name: v.vessel_name ?? "",
    imo_number: v.imo_number ?? "",
    dwt: s(v.dwt),
    year_built: s(v.year_built),
    flag: v.flag ?? "",
    class_society: v.class_society ?? "",
    loa: s(v.loa),
    beam: s(v.beam),
    max_draft: s(v.max_draft),
    cargo_capacity_cbm: s(v.cargo_capacity_cbm),
    number_of_tanks: s(v.number_of_tanks),
    tank_coating: v.tank_coating ?? "",
    pumps: v.pumps ?? "",
    heating_coils: v.heating_coils,
    imo_class: v.imo_class ?? "",
    sire_status: v.sire_status ?? "",
    cdi_status: v.cdi_status ?? "",
    last_3_cargoes: v.last_3_cargoes ?? "",
    trading_area: v.trading_area ?? "",
    opening_port: v.opening_port ?? "",
    opening_date: v.opening_date ?? "",
    status: v.status,
    remarks: v.remarks ?? "",
    q88_file_url: v.q88_file_url ?? null,
  };
}
...
export default function EditVessel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setForm(fromVessel(data as Vessel));
      } catch (e: any) {
  ...
  async function onSave() {
    ...
    try {
      ...
      const { error } = await supabase.from("vessels").update(payload).eq("id", id);
      if (error) throw error;
  ...
  function onDelete() {
    Alert.alert("Delete vessel?", "This cannot be undone.", [
    ...
    <View className="flex-1 bg-bg">
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }}>
        ...
        <Section title="Capacity">
          <Input label="DWT" value={form.dwt} onChangeText={(v) => set("dwt", v)} keyboardType="numeric" />
          <Input
            label="Year built"
            ...
            keyboardType="number-pad"
          />
          <Input label="LOA (m)" value={form.loa} onChangeText={(v) => set("loa", v)} keyboardType="numeric" />
          <Input label="Beam (m)" value={form.beam} onChangeText={(v) => set("beam", v)} keyboardType="numeric" />
          <Input
            label="Max draft (m)"
        ...
        <Section title="Equipment">
          ...
          />
          <Input label="Pumps" value={form.pumps} onChangeText={(v) => set("pumps", v)} />
          ...
          <Input label="IMO class" value={form.imo_class} onChangeText={(v) => set("imo_class", v)} />
        </Section>

        <Section title="Class & Inspection">
          <Input label="Flag" value={form.flag} onChangeText={(v) => set("flag", v)} />
          <Input
            label="Class society"
        ...
        <Section title="Q88 file">
          ...
          {form.q88_file_url ? (
            <Text className="text-xs text-emerald-700">Stored: {form.q88_file_url}</Text>
          ) : (
        ...
        <View className="mt-2 mb-3">
          <Button label="Save changes" onPress={onSave} loading={busy} fullWidth />
        </View>
        <Button label="Delete vessel" variant="danger" onPress={onDelete} fullWidth />
      </ScrollView>
    </View>
  );
}
